...
void main() {
  ...
  group('Populated RestApiPlaceVisitService', () {
    ...
    test('listPlaceVisits returns all place visits associated with a trip',
        () async {
      ...
      PlaceVisit createdPlaceVisit2 =
          await placeVisitService.createPlaceVisit(placeVisit2);
      expect(await placeVisitService.listPlaceVisits('abc'),
          unorderedEquals([createdPlaceVisit1, createdPlaceVisit2]));
    });
    ...
    test('getPlaceVisit returns placevisit for just added placevisit',
        () async {
      ...
      print(createdPlaceVisit.name);
      print(expectedPlaceVisit.name);

      expect(
          await placeVisitService.getPlaceVisit(
              createdPlaceVisit.tripid, createdPlaceVisit.id),
          equals(expectedPlaceVisit));
    });
    ...
    test('getPlaceVisit throws error for non-created id', () async {
      ...
      await placeVisitService.createPlaceVisit(placeVisit);
      expect(() async => await placeVisitService.getPlaceVisit('789', '123'),
          throwsException);
    });
    ...
    test('getPlaceVisit throws error for deleted place visit', () async {
      ...
      var createdPlaceVisit =
          await placeVisitService.createPlaceVisit(placeVisit);
      await placeVisitService.deletePlaceVisit(
          createdPlaceVisit.tripid, createdPlaceVisit.id);
      expect(
          () async => await placeVisitService.getPlaceVisit(
              createdPlaceVisit.tripid, createdPlaceVisit.id),
          throwsException);
    });

    test('updatePlaceVisitUserMark then get returns updated place visit',
        () async {
      ...
      var createdPlaceVisit =
          await placeVisitService.createPlaceVisit(originalPlaceVisit);
      ...
      var updatedPlaceVisit =
          await placeVisitService.updatePlaceVisitUserMark(newPlaceVisit);
